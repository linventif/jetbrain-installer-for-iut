...
config_file="$HOME/.datagrip-installer"
...
GenericName=Script permettant d'installer DataGrip
Exec=sh -c \"NOTIFY=1 $script_dir/$script_name install 2>&1 > $HOME/.datagrip-installer-auto-startup.log\"
Terminal=false
Type=Application
...
# Installe DataGrip en décompressant un fichier .tar.gz et en ajoutant un fichier .desktop
install () {
...
# Désinstalle DataGrip
remove () {
    echo "Supression fichiers DataGrip"
    rm -rf $path/datagrip 2> /dev/null
    echo "Suppression .desktop"
    rm $desktop_entry_path/datagrip.desktop 2> /dev/null
    rm $desktop_entry_auto_startup_path/install_datagrip.desktop 2> /dev/null
}

# Télécharge ou récupère le fichier .tar.gz contenant les fichiers de DataGrip
get_tar_gz_path () {
    if [[ $# == 1 ]]
    then
        use_user_tar_gz $1
...
# Installe DataGrip quand c'est utile (installation ou mise à jour)
download_if_useful() {
    ...
    then
        ...
        then
            echo "Une nouvelle version de DataGrip est disponible! ($latest_version_number)"
            notify "Mise à jour de DataGrip..."
        else
            notify "Installation de DataGrip..."
        fi
        use_downloaded_tar_gz
    else
        no_need_to_update
...
# Télécharge le tar.gz depuis le site de DataGrip
use_downloaded_tar_gz () {
    mkdir -p $path

    if [ -f $script_dir/datagrip-*-download.tar.gz ]
    then
        ...
        then
            get_latest_version_number
            tar_gz_path=$TAR_GZ_PATH/datagrip-$latest_version_number-download.tar.gz
        else
            tar_gz_path=$path/datagrip.tar.gz
        fi
...
use_already_downloaded_tar_gz() {
    current_tar_gz=$(ls $script_dir/datagrip-*-download.tar.gz | head -n 1)
    ...
    then
        ...
        rm $current_tar_gz
        tar_gz_path=$script_dir/datagrip-$latest_version_number-download.tar.gz
        download_latest_version $tar_gz_path
        is_downloaded=true
    fi
}
...
no_need_to_update () {
    echo "La dernière version de DataGrip est déjà installée!"
...
# Télécharge la dernière version de DataGrip
# Si il n'y a pas internet, un message d'erreur s'affiche
download_latest_version () {
    echo "Téléchargement de la dernière version de DataGrip..."
    curl -L "https://download.jetbrains.com/product?code=DG&latest&distribution=linux" -o $tar_gz_path
    if [[ $? != 0 ]]
    then
        printf "Impossible de télécharger la dernière version de DataGrip, essayez de fournir un fichier .tar.gz\n'$script_name install <filename>.tar.gz'\n"
...
# Extrait le fichier .tar.gz avec le nom de dossier "datagrip"
extract_tar_gz () {
    mkdir -p $path
    echo "Extraction de DataGrip à partir du fichier $tar_gz_path..."
    ...
    rm -rf $path/datagrip 2> /dev/null
    mv $path/DataGrip* $path/datagrip # Enlève le numéro de version à la fin du dossier
...
add_desktop_entry () {
    ...
    cat <<-EOF > $desktop_entry_path/datagrip.desktop
    [Desktop Entry]
    Version=1.0
    Type=Application
    Name=DataGrip
    Comment=The Database IDE
    Exec=$path/datagrip/bin/datagrip.sh %f
    Icon=$path/datagrip/bin/datagrip.svg
    Terminal=false
    StartupNotify=true
...
# Récupère le numéro de version de DataGrip qui est installée
get_installed_version_number () {
    installed_version_number=-1
    buildtxt_path=$path/datagrip/build.txt
    if [ -f $buildtxt_path ]
        then installed_version_number=$(cat $buildtxt_path | cut -d '-' -f 2)
    fi
}

# Récupère le dernier numéro de version de DataGrip
# Si il y a une erreur c'est égal à -1
get_latest_version_number () {
    latest_version_number=$(curl -s "https://data.services.jetbrains.com/products/releases?code=DG&latest=true&type=release" | grep -o '"build":"[^"]*' | grep -o '[^"]*$')
    if [ $? != 0 ]
    then
...
usage () {
    ...
    echo
    echo "Outil d'installation de l'IDE DataGrip sur les machines de l'IUT"
    echo
    echo "OBJECTS:"
    echo "  install                         Installe DataGrip dans le dossier $path"
    echo "  remove                          Désinstalle DataGrip (ne supprime pas les fichiers de configuration)"
    echo
    echo "OPTIONS:"
    echo "  -f                              Force la réinstallation de DataGrip"
